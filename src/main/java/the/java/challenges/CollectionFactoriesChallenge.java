...
public class CollectionFactoriesChallenge {
 public static void main(String... doYourBest) {
  ...
  try { dcHeros.add("Deadpool");
  } catch (UnsupportedOperationException e) {
      System.out.print("no space for Marvel heros here!|");
  }

  try { dcVillains.stream().collect(Collectors.toList()).add("Harley Quinn");
        finalDcCharacters.addAll(new ArrayList<>(dcVillains));
  } catch (UnsupportedOperationException | IllegalArgumentException e) {
      System.out.print("no space for villains"); System.exit(0);
  }

  try { var villains = Set.copyOf(dcVillains);
